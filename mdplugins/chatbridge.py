...
class mdplugin():
    def __init__(self, server: AeServer):
        ...
        self.chatbrige_id               = 1212613626435870781
    ...
    async def listener_on_message(self, message: discord.Message):
        if self.webhook is None: return
        if message.author.bot: return
        if message.channel.id == self.webhook.channel.id:
            if not (self.server.online_players + self.server.bots): return

            msg = message.content.replace('\n', ' ').replace('"', "'")
            self.server.send_response('@a', f'§9[Discord] §a<{message.author.display_name}>§f {msg}')
    ...
    async def on_player_message(self, player: str, message: str):
        ...
        await self.webhook.send(
            f'{message}',
            username = f'[{self.server.name}] {player}',
            avatar_url = f'https://mc-heads.net/head/{player.lower()}.png'
        )
