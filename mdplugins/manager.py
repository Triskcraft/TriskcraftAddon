...
class mdplugin():
    ...
    async def on_player_command(self, player: str, message: str):
        ...
        if self.server.is_command(message, 'online'):
            ...
            self.server.show_command(player, f"kill <process | default : {self.server.name}>", "Forzar el cierre del proceso.")
        ...
        elif self.server.is_command(message, 'kill'):
            arg = message.removeprefix(self.server.prefix + 'kill').strip().lower()
            if not arg:
                self.server.send_response(player, "✔ Matando proceso actual...")
                self.server.kill()
            else:
                if arg in [x.name.lower() for x in self.server.client.processes]:
                    process = next(filter(lambda x: arg == x.name.lower(), self.server.client.processes), None)
                    if not process.is_running():
                        self.server.send_response(player, f"✖ El proceso ya estaba cerrado...")
                        return
                    self.server.send_response(player, f"[{process.name}] Matando proceso...")
                    process.kill()
                else:
                    self.server.send_response(player, "✖ No hay un proceso con ese nombre.")
