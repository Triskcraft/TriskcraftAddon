...
LEFT_RE = re.compile(
    r'(?P<name>[A-Za-z0-9_]{3,16})\s+left\ the\ game',
    re.IGNORECASE
)
...
class AeServer(Server):
    def         __init__                (self, name: str, client: McDisClient, config: dict):
        ...
        self.admins                     = ['kys_us', 'Mirocabrera01','KillerVM','_DotJSON_','LSau', 'Busterbladerss','Severalplot4310','AkuaStille','TheVugx','TheLordPixc']
    ...
    async def   _listener_events        (self, log : str):
        if 'INFO]: ' in log:
            if any([f'<{player}>' in log for player in self.online_players]):
            ...
            elif 'left the game' in log.lower():
                # --- normalizar log ---
                text = COLOR_CODE_RE.sub('', log)
                text = ANSI_RE.sub('', text)
                text = unicodedata.normalize('NFKC', text)
                text = ' '.join(text.split())
                log_normalized = text.strip()

                # --- extraer jugador ---
                match = LEFT_RE.search(log_normalized)
                if not match:
                    player = None  # log roto o formato no estandar
                else: 
                ...
                elif player in self.online_players:
                    self.online_players.remove(player)
                # TODO? Modificado el 24/01/2026 por eli para evitar `None` en los mensajes
                # else:
                #     player = None # jugador no registrado previamente
